...
pub fn update_dataset_info(base_dir: Option<PathBuf>, dataset_name: &str, new_format: Option<&str>, new_category: Option<&str>, new_dataset_file_path: Option<PathBuf>, add_new_dataset: bool,) -> io::Result<()>
{
    ...
    if let Some(dataset) = config.datasets.get_mut(dataset_name) {
        ...
        config.datasets.insert(dataset_name.to_string(), new_dataset);
    } else {
        // If the dataset does not exist and the user does not want to add a new one
        eprintln!("Dataset '{}' does not exist and 'add_new_dataset' is false.", dataset_name);
...
pub fn search_dataset_by_category(file_path: PathBuf,category: &str,) -> io::Result<HashMap<String, Dataset>>  
{
    // Read the TOML file into a string
...
pub fn get_dataset_file_paths(dataset_info_path: PathBuf, category_filter: Option<&str>) -> Result<Vec<PathBuf>, Box<dyn std::error::Error>> {
    // Read the TOML file
    ...
    let file_paths: Vec<PathBuf> = config.datasets.values()
        .filter(|dataset| {
...
mod tests {
    ...
    fn test_create_dataset_info() {
        ...
        assert!(config.datasets.is_empty(), "Initial datasets list should be empty");
    }
    ...
    fn test_update_dataset_info_add_new_dataset() {
        ...
        assert!(config.datasets.contains_key("test_dataset"), "Dataset should be added");
        let dataset = config.datasets.get("test_dataset").unwrap();
        assert_eq!(dataset.format, "json", "Dataset format should be updated");
        assert!(dataset.categories.contains(&"test_category".to_string()), "Category should be added");
        assert_eq!(dataset.filepath, PathBuf::from("test/path"), "Filepath should be updated");
    }
    ...
    fn test_update_dataset_info_update_existing_dataset() {
        ...
        assert!(dataset.categories.contains(&"new_category".to_string()), "New category should be added");
    }
    ...
    fn test_search_dataset_by_category() {
        ...
        assert!(matching_datasets.contains_key("dataset1"), "Matching dataset should be 'dataset1'");
    }
