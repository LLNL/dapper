...
pub fn normalize_file_name(name: &str) -> NormalizedFileName {
                ...
                .any(|suffix| name.ends_with(suffix)))
    {
        ...
        // Decide if the "Kernels" ending in ".hsaco" need to be filtered out (or this entire category should be ignored)
        let base_key = format!("{}.so", name.split(".so-").next().unwrap());
        return NormalizedFileName::Normalized(base_key);
    } else if name.contains(".so_") {
        // Decide if this category should be included -- basically just lib_postgresqludf_sys.so_
        let base_key = format!("{}.so", name.split(".so_").next().unwrap());
        return NormalizedFileName::Normalized(base_key);
    }
    NormalizedFileName::Unchanged
}

pub fn normalize_and_check_dataset_file_names() {
